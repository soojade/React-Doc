...
```
JSX 语法是可选的，但是我们发现 JSX 语句比纯 JavaScript 用起来更容易。更多内容请阅读[《 JSX 语法介绍》](深入理解JSX.md)。
...
因此，我们将使用`this.state`来保存用户的输入。我们通过两个属性`author`和`text`来初始化`state`，并且设置为空字符串。在我们`<input>`元素中，我们设置`value`属性来映射组件的`state`，并绑定`onChange`处理器。这种带有`value`的`input`元素被称为控制组件。阅读更多关于受限组件的内容查看[表单章节](表单.md#受限组件)。
```
// tutorial16.js
var CommentForm = React.createClass({
  getInitialState: function() {
...
```
### 祝贺你!
你刚刚通过一些简单步骤构造了一个评论框。了解更多关于[为什么使用 React](为什么使用 React.md)，或者深入学习 [API 参考](顶层API.md)，开始专研！祝你好运！