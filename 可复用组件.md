...
React.createClass({
  propTypes: {
    ...
    // 你也可以给 `arrayOf`和 `objectOf`提供自定义验证器。
    // 如果验证失败，会返回一个错误对象。
    // 验证器会被每一个数组或对象的 键 调用。第一次验证器的两个参数是数组或对象本身，
    // 然后是一系列的 键。
...
一个通用的场景是：一个组件需要定期更新。用 `setInterval() `做很容易，但当不需要它的时候取消定时器来节省内存是非常重要的。React 提供 [生命周期方法](浏览器中的工作原理.md#组件声明周期) 来告知组件创建或销毁的时间。下面来做一个简单的 mixin，使用 `setInterval()` 并保证在组件销毁时清理定时器。
```
...
var TickTock = React.createClass({
  mixins: [SetIntervalMixin], //  调用 mixin 的方法
...
## ES6 Classes
你可以定义你的React classes 作为你的纯javascript 类，例如使用ES6 class 语法：
```
