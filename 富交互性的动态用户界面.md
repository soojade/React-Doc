# 富交互性的动态用户界面
我们已经学习如何使用 React [呈现数据](数据呈现.md)。现在让我们来学习如何创建交互式界面。
...
* **计算所得数据**： 不要担心根据 state 来预先计算数据 —— 把所有的计算都放到 `render()` 里更容易保证用户界面和数据的一致性。例如，在 state 里有一个数组（listItems），我们要把数组长度渲染成字符串， 直接在 `render()`使用 `this.state.listItems.length + ' list items'` 比把它放到 state 里好的多。
